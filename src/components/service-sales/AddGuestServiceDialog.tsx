...
export const AddGuestServiceDialog = ({ open, onOpenChange, defaultGuestId }: AddGuestServiceDialogProps) => {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const finalPrice = isFreeTrial ? 0 : price;
  ...
  useEffect(() => {
    if (selectedService) {
      const servicePrice = isFreeTrial ? 0 : selectedService.price;
      setPrice(servicePrice);
      setFormattedPrice(new Intl.NumberFormat('vi-VN').format(servicePrice));
      setPaidAmount(servicePrice);
      setFormattedPaidAmount(new Intl.NumberFormat('vi-VN').format(servicePrice));
    } else {
      setPrice(0);
      setFormattedPrice("0");
      setPaidAmount(0);
      ...
      <DialogContent>
        ...
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            ...
            <Popover open={isGuestPopoverOpen} onOpenChange={setIsGuestPopoverOpen}><PopoverTrigger asChild><Button variant="outline" role="combobox" className="w-full justify-between" disabled={!!defaultGuestId}>{selectedGuestId ? guests.find(g => g.id === selectedGuestId)?.name : "Chọn khách mời..."}<ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" /></Button></PopoverTrigger><PopoverContent side="bottom" align="start" className="w-[--radix-popover-trigger-width] p-0" avoidCollisions={false}><Command><CommandInput placeholder="Tìm khách..." /><CommandList onWheel={(e) => e.stopPropagation()}><CommandEmpty>Không tìm thấy.</CommandEmpty><CommandGroup>{guests.map(guest => (<CommandItem value={guest.name} key={guest.id} onSelect={() => { setSelectedGuestId(guest.id); setIsGuestPopoverOpen(false); }}><Check className={cn("mr-2 h-4 w-4", guest.id === selectedGuestId ? "opacity-100" : "opacity-0")} />{guest.name}</CommandItem>))}</CommandGroup></CommandList></Command></PopoverContent></Popover>
          </div>
          <div className="space-y-2">
            <Label>Dịch vụ</Label>
          ...
          {selectedService?.allow_free_trial && (
            <div className="flex items-center space-x-2">
              <Checkbox id="free-trial" checked={isFreeTrial} onCheckedChange={(checked) => setIsFreeTrial(!!checked)} />
              <Label htmlFor="free-trial">Trải nghiệm free</Label>
            </div>
          )}
          <div className="space-y-2">
