  ...
  const combinedSummary = useMemo((): CombinedCommissionSummary[] => {
    ...
    upsaleSummaryData.forEach(item => {
      summaryMap.set(item.user_id, {
        ...
        upsaleCount: parseInt(String(item.upsale_count || 0), 10),
    ...
    serviceSummary.forEach(item => {
      if (item.referrer_type === 'sale') {
        ...
        if (existing) {
          existing.serviceCount = parseInt(String(item.service_count || 0), 10);
          ...
          summaryMap.set(item.referrer_id, {
            ...
            totalUpsaleAmount: 0,
            upsaleCommission: 0,
            serviceCount: parseInt(String(item.service_count || 0), 10),
