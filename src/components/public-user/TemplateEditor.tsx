...
export const TemplateEditor = ({ content, onContentChange, onUploadingChange }: TemplateEditorProps) => {
                  ...
                  <div className="w-full">
                    ...
                    <div className="flex gap-4 items-start">
                              ...
                              <div key={item.id} style={{ marginTop: `${item.marginTop}px`, marginRight: `${item.marginRight}px`, marginBottom: `${item.marginBottom}px`, marginLeft: `${item.marginLeft}px` }}>
                                {item.type === 'text' ? (
                                  <p className="text-sm text-center break-words" style={{ color: item.color, fontSize: `${item.fontSize}px`, fontWeight: item.fontWeight, fontStyle: item.fontStyle, fontFamily: item.fontFamily }}>
                                    {item.isGuestName ? mockGuest.name : item.text}
                                  </p>
                                ) : (
                      ...
                      <div className="w-2/3 flex-grow space-y-2">
                        {block.type === 'image' && (
                          <>
                            <RadioGroup value={block.imageSourceType || 'url'} onValueChange={(value) => handleUpdateBlock(block.id, 'imageSourceType', value)} className="flex gap-4"><Label htmlFor={`image-url-${block.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="url" id={`image-url-${block.id}`} /><span>Nhập link</span></Label><Label htmlFor={`image-upload-${block.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="upload" id={`image-upload-${block.id}`} /><span>Tải ảnh</span></Label></RadioGroup>
                            {(block.imageSourceType === 'url' || !block.imageSourceType) ? <Input placeholder="Link ảnh" value={block.imageUrl} onChange={e => handleUpdateBlock(block.id, 'imageUrl', e.target.value)} className="truncate" /> : <ImageUploader guestId={mockGuest.id} onUploadSuccess={url => handleUpdateBlock(block.id, 'imageUrl', url)} onUploading={isUp => handleUploadingState(`block-image-${block.id}`, isUp)} />}
                        ...
                        {block.type === 'text' && (
                          <>
                            ...
                            <RadioGroup value={block.imageSourceType || 'url'} onValueChange={(value) => handleUpdateBlock(block.id, 'imageSourceType', value)} className="flex gap-4"><Label htmlFor={`bg-url-${block.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="url" id={`bg-url-${block.id}`} /><span>Nhập link</span></Label><Label htmlFor={`bg-upload-${block.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="upload" id={`bg-upload-${block.id}`} /><span>Tải ảnh</span></Label></RadioGroup>
                            {(block.imageSourceType === 'url' || !block.imageSourceType) ? <Input placeholder="Link ảnh nền (tùy chọn)" value={block.backgroundImageUrl} onChange={e => handleUpdateBlock(block.id, 'backgroundImageUrl', e.target.value)} className="truncate" /> : <ImageUploader guestId={mockGuest.id} onUploadSuccess={url => handleUpdateBlock(block.id, 'backgroundImageUrl', url)} onUploading={isUp => handleUploadingState(`block-bg-${block.id}`, isUp)} />}
                                  ...
                                  <SortableItem key={item.id} id={`item_${block.id}_${item.id}`} className="p-2 border rounded-md bg-white relative">
                                    ...
                                    {item.type === 'text' ? (
                                      <div className="space-y-2">
                                        <RadioGroup value={item.isGuestName ? 'auto' : 'manual'} onValueChange={(value) => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, isGuestName: value === 'auto'} : i))} className="flex gap-4"><Label htmlFor={`text-manual-${item.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="manual" id={`text-manual-${item.id}`} /><span>Text nhập</span></Label><Label htmlFor={`text-auto-${item.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="auto" id={`text-auto-${item.id}`} /><span>Tên khách mời</span></Label></RadioGroup>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                          <div><Label className="text-xs">Font</Label><Select value={item.fontFamily || 'sans-serif'} onValueChange={value => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, fontFamily: value} : i))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{fontFamilies.map(f => <SelectItem key={f.value} value={f.value}>{f.label}</SelectItem>)}</SelectContent></Select></div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                          <div><Label className="text-xs">Cỡ chữ</Label><Input type="number" value={item.fontSize || 32} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, fontSize: Number(e.target.value)} : i))} /></div>
                                          <div><Label className="text-xs">Màu chữ</Label><Input type="color" value={item.color || '#FFFFFF'} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, color: e.target.value} : i))} className="p-1 h-10" /></div>
                                        </div>
                                        <div className="grid grid-cols-4 gap-1">
                                          <div><Label className="text-xs">M.Top</Label><Input type="number" value={item.marginTop || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginTop: Number(e.target.value)} : i))} /></div>
                                          <div><Label className="text-xs">M.Right</Label><Input type="number" value={item.marginRight || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginRight: Number(e.target.value)} : i))} /></div>
                                          <div><Label className="text-xs">M.Bottom</Label><Input type="number" value={item.marginBottom || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginBottom: Number(e.target.value)} : i))} /></div>
                                          <div><Label className="text-xs">M.Left</Label><Input type="number" value={item.marginLeft || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginLeft: Number(e.target.value)} : i))} /></div>
                                        </div>
                                      </div>
                                    ) : (
                                      <div className="space-y-2">
                                        <RadioGroup value={item.imageSourceType || 'url'} onValueChange={(value) => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, imageSourceType: value} : i))} className="flex gap-4"><Label htmlFor={`item-img-url-${item.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="url" id={`item-img-url-${item.id}`} /><span>Nhập link</span></Label><Label htmlFor={`item-img-upload-${item.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="upload" id={`item-img-upload-${item.id}`} /><span>Tải ảnh</span></Label></RadioGroup>
                                        {(item.imageSourceType === 'url' || !item.imageSourceType) ? <Input placeholder="Link ảnh" value={item.imageUrl} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, imageUrl: e.target.value} : i))} /> : <ImageUploader guestId={mockGuest.id} onUploadSuccess={url => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, imageUrl: url} : i))} onUploading={isUp => handleUploadingState(`item-image-${item.id}`, isUp)} />}
                                        <div><Label className="text-xs">Rộng (%)</Label><Slider value={[item.width || 100]} onValueChange={([val]) => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, width: val} : i))} max={100} step={1} /></div>
                                        <div className="grid grid-cols-4 gap-1">
                                          <div><Label className="text-xs">M.Top</Label><Input type="number" value={item.marginTop || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginTop: Number(e.target.value)} : i))} /></div>
                                          <div><Label className="text-xs">M.Right</Label><Input type="number" value={item.marginRight || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginRight: Number(e.target.value)} : i))} /></div>
                                          <div><Label className="text-xs">M.Bottom</Label><Input type="number" value={item.marginBottom || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginBottom: Number(e.target.value)} : i))} /></div>
