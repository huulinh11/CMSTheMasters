                    ...
                    <div className="w-full">
                      ...
                      <div className="flex gap-4 items-start">
                        ...
                        <div className="w-2/3 flex-grow space-y-2">
                          {block.type === 'image' && (
                            <>
                              <RadioGroup value={block.imageSourceType || 'url'} onValueChange={(value) => handleUpdateBlock(block.id, 'imageSourceType', value)} className="flex gap-4" disabled={isTemplateMode}>
                                <Label htmlFor={`image-url-${block.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="url" id={`image-url-${block.id}`} /><span>Nhập link</span></Label>
                                <Label htmlFor={`image-upload-${block.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="upload" id={`image-upload-${block.id}`} /><span>Tải ảnh</span></Label>
                              </RadioGroup>
                          ...
                          {block.type === 'text' && (
                            <>
                              ...
                              <RadioGroup value={block.imageSourceType || 'url'} onValueChange={(value) => handleUpdateBlock(block.id, 'imageSourceType', value)} className="flex gap-4" disabled={isTemplateMode}>
                                <Label htmlFor={`bg-url-${block.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="url" id={`bg-url-${block.id}`} /><span>Nhập link</span></Label>
                                <Label htmlFor={`bg-upload-${block.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="upload" id={`bg-upload-${block.id}`} /><span>Tải ảnh</span></Label>
                              </RadioGroup>
                                    ...
                                    <SortableItem key={item.id} id={`item_${block.id}_${item.id}`} className="p-2 border rounded-md bg-white relative">
                                      ...
                                      {item.type === 'text' ? (
                                        <div className="space-y-2">
                                          <RadioGroup value={item.isGuestName ? 'auto' : 'manual'} onValueChange={(value) => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, isGuestName: value === 'auto'} : i))} className="flex gap-4" disabled={isTemplateMode}>
                                            <Label htmlFor={`text-manual-${item.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="manual" id={`text-manual-${item.id}`} /><span>Text nhập</span></Label>
                                            <Label htmlFor={`text-auto-${item.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="auto" id={`text-auto-${item.id}`} /><span>Tên khách mời</span></Label>
                                          </RadioGroup>
                                        ...
                                        <div className="space-y-2">
                                          <RadioGroup value={item.imageSourceType || 'url'} onValueChange={(value) => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, imageSourceType: value} : i))} className="flex gap-4" disabled={isTemplateMode}>
                                            <Label htmlFor={`item-img-url-${item.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="url" id={`item-img-url-${item.id}`} /><span>Nhập link</span></Label>
                                            <Label htmlFor={`item-img-upload-${item.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="upload" id={`item-img-upload-${item.id}`} /><span>Tải ảnh</span></Label>
                                          </RadioGroup>
