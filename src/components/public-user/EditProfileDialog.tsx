      ...
      const migratedBlocks = rawBlocks.map(block => {
        ...
        if (block.type === 'text') {
          ...
          if (textBlock.items) {
            ...
            items = textBlock.texts.map((t: any) => ({ ...t, type: 'text', id: t.id || uuidv4() }));
          }
                    ...
                    <div className="w-full">
                      ...
                      <div className="flex gap-4 items-start">
                          ...
                          {block.type === 'text' && (() => {
                            ...
                            return (
                              <div className={`w-full rounded-md border bg-cover bg-center flex flex-col items-center justify-start p-1 ${!style.aspectRatio ? 'aspect-video bg-slate-100' : ''}`} style={style}>
                                {block.items.map(item => (<div key={item.id} style={{ marginTop: `${item.marginTop}px`, marginRight: `${item.marginRight}px`, marginBottom: `${item.marginBottom}px`, marginLeft: `${item.marginLeft}px` }}>{item.type === 'text' ? (<p className="text-sm text-center break-words" style={{ color: item.color, fontSize: `${item.fontSize}px`, fontWeight: item.fontWeight, fontStyle: item.fontStyle, fontFamily: item.fontFamily, textTransform: item.isCaps ? 'uppercase' : 'none' }}>{item.isGuestName ? "Tên khách mời" : (item.isGuestRole ? "Vai trò" : item.text)}</p>) : (<img src={item.imageUrl} alt="item" style={{ width: `${item.width}%`, margin: '0 auto' }} />)}</div>))}
                              </div>
                            );
                          })()}
                        </div>
                        ...
                        <div className="w-2/3 flex-grow space-y-2">
                          {block.type === 'image' && (<><ImageSourceSelector value={block.imageUrl} onValueChange={url => handleUpdateBlock(block.id, 'imageUrl', url)} onUploadingChange={isUp => handleUploadingState(`block-image-${block.id}`, isUp)} /><Input placeholder="Link khi click (tùy chọn)" value={block.linkUrl} onChange={e => handleUpdateBlock(block.id, 'linkUrl', e.target.value)} className="truncate" /><div><Label className="text-xs">Rộng (%)</Label><Slider value={[block.width || 100]} onValueChange={([val]) => handleUpdateBlock(block.id, 'width', val)} max={100} step={1} disabled={isTemplateMode} /></div></>)}
                          {block.type === 'video' && (<><Input placeholder="Link Youtube hoặc Facebook" value={block.videoUrl} onChange={e => handleUpdateBlock(block.id, 'videoUrl', e.target.value)} className="truncate" /><div className="flex items-center gap-2"><div className="flex-1 space-y-1"><Label htmlFor={`aspectWidth-${block.id}`} className="text-xs">Rộng</Label><Input id={`aspectWidth-${block.id}`} type="number" placeholder="16" value={block.aspectWidth || ''} onChange={e => handleUpdateBlock(block.id, 'aspectWidth', e.target.value ? Number(e.target.value) : undefined)} disabled={isTemplateMode} /></div><div className="flex-1 space-y-1"><Label htmlFor={`aspectHeight-${block.id}`} className="text-xs">Cao</Label><Input id={`aspectHeight-${block.id}`} type="number" placeholder="9" value={block.aspectHeight || ''} onChange={e => handleUpdateBlock(block.id, 'aspectHeight', e.target.value ? Number(e.target.value) : undefined)} disabled={isTemplateMode} /></div></div></>)}
                          {block.type === 'text' && (<><Label className="font-medium text-sm">Ảnh nền</Label><ImageSourceSelector value={block.backgroundImageUrl} onValueChange={url => handleUpdateBlock(block.id, 'backgroundImageUrl', url)} onUploadingChange={isUp => handleUploadingState(`block-bg-${block.id}`, isUp)} /><div className="flex items-center space-x-2"><Checkbox id={`useImageDimensions-${block.id}`} checked={block.useImageDimensions} onCheckedChange={(checked) => handleUpdateBlock(block.id, 'useImageDimensions', !!checked)} disabled={isTemplateMode} /><Label htmlFor={`useImageDimensions-${block.id}`} className="text-sm font-medium">Lấy kích thước của ảnh</Label></div><Separator className="my-3" /><Label className="font-medium text-sm">Nội dung</Label><SortableContext items={block.items.map(t => `item_${block.id}_${t.id}`)} strategy={verticalListSortingStrategy}><div className="space-y-3">{block.items.map((item) => (<SortableItem key={item.id} id={`item_${block.id}_${item.id}`} className="p-2 border rounded-md bg-white relative"><Button variant="ghost" size="icon" className="absolute top-0 right-0 h-7 w-7" onClick={() => handleUpdateBlock(block.id, 'items', block.items.filter(i => i.id !== item.id))} disabled={isTemplateMode}><Trash2 className="h-4 w-4 text-red-500" /></Button>{item.type === 'text' ? (<div className="space-y-2"><RadioGroup value={item.isGuestName ? 'guestName' : item.isGuestRole ? 'guestRole' : 'manual'} onValueChange={(value) => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, isGuestName: value === 'guestName', isGuestRole: value === 'guestRole'} : i))} className="flex gap-4 flex-wrap" disabled={isTemplateMode}><Label htmlFor={`text-manual-${item.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="manual" id={`text-manual-${item.id}`} /><span>Text nhập</span></Label><Label htmlFor={`text-name-${item.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="guestName" id={`text-name-${item.id}`} /><span>Tên khách mời</span></Label><Label htmlFor={`text-role-${item.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="guestRole" id={`text-role-${item.id}`} /><span>Vai trò</span></Label></RadioGroup><div className="flex items-center space-x-2"><Checkbox id={`caps-lock-profile-${item.id}`} checked={item.isCaps} onCheckedChange={(checked) => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, isCaps: !!checked} : i))} disabled={isTemplateMode} /><Label htmlFor={`caps-lock-profile-${item.id}`} className="text-sm font-medium">Caps Lock</Label></div>{!item.isGuestName && !item.isGuestRole && <Textarea placeholder="Nội dung text" value={item.text} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, text: e.target.value} : i))} rows={2} />}<div className="grid grid-cols-2 gap-2"><div><Label className="text-xs">Font</Label><Select value={item.fontFamily || 'sans-serif'} onValueChange={value => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, fontFamily: value} : i))} disabled={isTemplateMode}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{fontFamilies.map(f => <SelectItem key={f.value} value={f.value}>{f.label}</SelectItem>)}</SelectContent></Select></div><div><Label className="text-xs">Kiểu</Label><Select value={`${item.fontWeight || 'bold'}-${item.fontStyle || 'normal'}`} onValueChange={value => { const [fontWeight, fontStyle] = value.split('-'); handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, fontWeight, fontStyle} : i))}} disabled={isTemplateMode}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="normal-normal">Thường</SelectItem><SelectItem value="bold-normal">Đậm</SelectItem><SelectItem value="normal-italic">Nghiêng</SelectItem><SelectItem value="bold-italic">Đậm Nghiêng</SelectItem></SelectContent></Select></div></div><div className="grid grid-cols-2 gap-2"><div><Label className="text-xs">Cỡ chữ</Label><Input type="number" value={item.fontSize || 32} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, fontSize: Number(e.target.value)} : i))} disabled={isTemplateMode} /></div><div><Label className="text-xs">Màu chữ</Label><Input type="color" value={item.color || '#FFFFFF'} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, color: e.target.value} : i))} className="p-1 h-10" disabled={isTemplateMode} /></div></div><div className="grid grid-cols-4 gap-1"><div><Label className="text-xs">M.Top</Label><Input type="number" value={item.marginTop || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginTop: Number(e.target.value)} : i))} disabled={isTemplateMode} /></div><div><Label className="text-xs">M.Right</Label><Input type="number" value={item.marginRight || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginRight: Number(e.target.value)} : i))} disabled={isTemplateMode} /></div><div><Label className="text-xs">M.Bottom</Label><Input type="number" value={item.marginBottom || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginBottom: Number(e.target.value)} : i))} disabled={isTemplateMode} /></div><div><Label className="text-xs">M.Left</Label><Input type="number" value={item.marginLeft || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginLeft: Number(e.target.value)} : i))} disabled={isTemplateMode} /></div></div></div>) : (<div className="space-y-2"><ImageSourceSelector value={item.imageUrl} onValueChange={url => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, imageUrl: url} : i))} onUploadingChange={isUp => handleUploadingState(`item-image-${item.id}`, isUp)} /><div><Label className="text-xs">Rộng (%)</Label><Slider value={[item.width || 100]} onValueChange={([val]) => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, width: val} : i))} max={100} step={1} disabled={isTemplateMode} /></div><div className="grid grid-cols-4 gap-1"><div><Label className="text-xs">M.Top</Label><Input type="number" value={item.marginTop || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginTop: Number(e.target.value)} : i))} disabled={isTemplateMode} /></div><div><Label className="text-xs">M.Right</Label><Input type="number" value={item.marginRight || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginRight: Number(e.target.value)} : i))} disabled={isTemplateMode} /></div><div><Label className="text-xs">M.Bottom</Label><Input type="number" value={item.marginBottom || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginBottom: Number(e.target.value)} : i))} disabled={isTemplateMode} /></div><div><Label className="text-xs">M.Left</Label><Input type="number" value={item.marginLeft || 0} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, marginLeft: Number(e.target.value)} : i))} disabled={isTemplateMode} /></div></div></div>)}</SortableItem>))}</div></SortableContext><div className="flex gap-2 mt-2"><Button type="button" variant="outline" size="sm" onClick={() => handleUpdateBlock(block.id, 'items', [...block.items, { id: uuidv4(), type: 'text', text: 'Nội dung mới', isGuestName: false, isGuestRole: false, isCaps: false, fontSize: 32, color: '#FFFFFF', fontWeight: 'bold', fontStyle: 'normal', fontFamily: 'sans-serif', marginTop: 0, marginRight: 0, marginBottom: 0, marginLeft: 0 }])} disabled={isTemplateMode}><PlusCircle className="mr-2 h-4 w-4" /> Thêm Text</Button><Button type="button" variant="outline" size="sm" onClick={() => handleUpdateBlock(block.id, 'items', [...block.items, { id: uuidv4(), type: 'image', imageUrl: '', imageSourceType: 'url', width: 100, marginTop: 10, marginRight: 0, marginBottom: 0, marginLeft: 0 }])} disabled={isTemplateMode}><ImageIcon className="mr-2 h-4 w-4" /> Thêm Ảnh</Button></div></>)}
                        </div>
                      </div>
                    </div>
                  </SortableItem>
