                    ...
                    <div className="w-full">
                      ...
                      <div className="flex gap-4 items-start">
                        ...
                        <div className="w-2/3 flex-grow space-y-2">
                          {block.type === 'image' && (
                            <>
                              <RadioGroup value={block.imageSourceType || 'url'} onValueChange={(value) => handleUpdateBlock(block.id, 'imageSourceType', value)} className="flex gap-4" disabled={isTemplateMode}><Label htmlFor={`image-url-${block.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="url" id={`image-url-${block.id}`} /><span>Nhập link</span></Label><Label htmlFor={`image-upload-${block.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="upload" id={`image-upload-${block.id}`} /><span>Tải ảnh</span></Label></RadioGroup>
                              {(block.imageSourceType === 'url' || !block.imageSourceType) ? <Input placeholder="Link ảnh" value={block.imageUrl} onChange={e => handleUpdateBlock(block.id, 'imageUrl', e.target.value)} className="truncate" /> : <ImageUploader onUploadSuccess={url => handleUpdateBlock(block.id, 'imageUrl', url)} onUploading={isUp => handleUploadingState(`block-image-${block.id}`, isUp)} />}
                          ...
                          {block.type === 'text' && (
                            <>
                              ...
                              <RadioGroup value={block.imageSourceType || 'url'} onValueChange={(value) => handleUpdateBlock(block.id, 'imageSourceType', value)} className="flex gap-4" disabled={isTemplateMode}><Label htmlFor={`bg-url-${block.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="url" id={`bg-url-${block.id}`} /><span>Nhập link</span></Label><Label htmlFor={`bg-upload-${block.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="upload" id={`bg-upload-${block.id}`} /><span>Tải ảnh</span></Label></RadioGroup>
                              {(block.imageSourceType === 'url' || !block.imageSourceType) ? <Input placeholder="Link ảnh nền (tùy chọn)" value={block.backgroundImageUrl} onChange={e => handleUpdateBlock(block.id, 'backgroundImageUrl', e.target.value)} className="truncate" disabled={isTemplateMode} /> : <ImageUploader onUploadSuccess={url => handleUpdateBlock(block.id, 'backgroundImageUrl', url)} onUploading={isUp => handleUploadingState(`block-bg-${block.id}`, isUp)} />}
                                  ...
                                  {block.items.map((item) => (
                                    <SortableItem key={item.id} id={`item_${block.id}_${item.id}`} className="p-2 border rounded-md bg-white relative">
                                      ...
                                      {item.type === 'text' ? (
                                        ...
                                        <div className="space-y-2">
                                          <RadioGroup value={item.imageSourceType || 'url'} onValueChange={(value) => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, imageSourceType: value} : i))} className="flex gap-4" disabled={isTemplateMode}><Label htmlFor={`item-img-url-${item.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="url" id={`item-img-url-${item.id}`} /><span>Nhập link</span></Label><Label htmlFor={`item-img-upload-${item.id}`} className="flex items-center space-x-2 cursor-pointer"><RadioGroupItem value="upload" id={`item-img-upload-${item.id}`} /><span>Tải ảnh</span></Label></RadioGroup>
                                          {(item.imageSourceType === 'url' || !item.imageSourceType) ? <Input placeholder="Link ảnh" value={item.imageUrl} onChange={e => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, imageUrl: e.target.value} : i))} /> : <ImageUploader onUploadSuccess={url => handleUpdateBlock(block.id, 'items', block.items.map(i => i.id === item.id ? {...i, imageUrl: url} : i))} onUploading={isUp => handleUploadingState(`item-image-${item.id}`, isUp)} />}
