...
// Re-implementing necessary components locally to avoid prop drilling issues
...
export const EditTemplateDialog = ({ open, onOpenChange, onSave, isSaving, template, allRoles }: EditTemplateDialogProps) => {
  ...
  const handleSave = () => {
    onSave({ id: template?.id, name, assigned_role: assignedRole, content: blocks });
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
          ...
          <div><Label htmlFor="assigned-role">Gán cho vai trò (mặc định)</Label><Select value={assignedRole || "NONE"} onValueChange={(value) => setAssignedRole(value === "NONE" ? null : value)}><SelectTrigger id="assigned-role"><SelectValue placeholder="Chọn vai trò" /></SelectTrigger><SelectContent><SelectItem value="NONE">Không có</SelectItem>{allRoles.map((role) => (<SelectItem key={role.id} value={role.name}>{role.name}</SelectItem>))}</SelectContent></Select></div>
        </div>
