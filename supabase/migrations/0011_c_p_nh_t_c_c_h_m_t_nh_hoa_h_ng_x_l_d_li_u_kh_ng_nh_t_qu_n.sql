-- Update get_upsale_commission_summary to be case-insensitive
...
BEGIN
    ...
    all_sales_users AS (
        SELECT TRIM(full_name) as full_name FROM public.profiles WHERE role = 'Sale'
    )
    SELECT
    ...
    FULL OUTER JOIN aggregated_upsales au ON lower(trim(asu.full_name)) = lower(trim(au.upsale_person_name));
END;
$$;
